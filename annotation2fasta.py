...
for i in range(len(trans_name)):
    if transcript_seq[i]=="NULL":
        ...
        if (consequence[i] == "missense_variant") or (consequence[i] == "MISSENSE"):
            ...
            wt_head='>INDEL_'+ str(output_line_num[i])
            mt_head='>INDEL_'+ str(output_line_num[i])+"_"+str(tpm_num[i])
            ...
            if consequence[i] == "frameshift_variant":
                if (len(from_base)>len(to_base)) or (to_base=="-"): # the frameshift is due to deletion
                    dna_seq = ref_fasta[chr][start_pos-frame_shift_num:start_pos]+ref_fasta[chr][end_pos:start_pos+length-frame_shift_num]
                    aa_seq = translate(dna_seq)
                elif (len(from_base)<len(to_base)) or (from_base=="-"): # the frameshift is due to insertion       
                    dna_seq = ref_fasta[chr][start_pos-frame_shift_num:start_pos]+to_base+ref_fasta[chr][start_pos:start_pos+length-frame_shift_num]
                    aa_seq = translate(dna_seq)
                else:
                    print("[WARNING] Wrong output consequence")
